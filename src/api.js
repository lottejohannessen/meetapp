...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch(
    `https://cq9p1e4xp4.execute-api.ca-central-1.amazonaws.com/dev/api/token/${encodeCode}` 
  )
    .then((res) => {
      return res.json();
    })
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url = `https://cq9p1e4xp4.execute-api.ca-central-1.amazonaws.com/dev/api/get-events/${token}`
